...
<div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0">Изменение Трека</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
    ...
    <section class="content">
        ...
        <div class="card card-white">
            ...
            <!-- form start -->
            <form action="{{route('track.update', $track)}}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PATCH')
              <div class="card-body">
{{--
                <div class="row">
                    <div class="block_one">
                        ...
                        <input type="text" class="form-control" id="name" placeholder="Введите Название" name="title" value="{{$track->title}}">
                        @error('title')
                ...
                <div class="row">
                    <div class="text">
                        ...
                        <textarea class="form-control" id="lyrics" name="lyrics" placeholder="Введите текст" rows="6">{{$track->lyrics}}</textarea>
                        @error('lyrics')
                        <p class="text-danger">{{$message}}</p>
                      @enderror
                      </div>
                    ...
                    <div class="block_one">
                      ...
                      <div class="block_one">
                        ...
                        <div class="block_one">
                            ...
                            <input type="number" class="form-control" id="number" placeholder="Введите номер трекв" name="track_number" value="{{$track->track_number}}">
                            @error('track_number')
                            <p class="text-danger">{{$message}}</p>
                            @enderror
                          </div>
                ...
                <div class="row">
                    <div class="block_one">
                        <label for="status">Статуc</label>
                          <input type="checkbox" name="status" id="status" data-bootstrap-switch {{$track->status == true ? 'checked' : ''}}>
                          @error('status')
                          <p class="text-danger">{{$message}}</p>
                        @enderror
                      </div>
                      <div class="block_one">
                        <label for="is_national">Национальная</label>
                          <input type="checkbox" name="is_national" id="is_national" data-bootstrap-switch {{$track->is_national == true ? 'checked' : ''}}>
                          @error('is_national')
                          <p class="text-danger">{{$message}}</p>
                        @enderror
                      </div>
