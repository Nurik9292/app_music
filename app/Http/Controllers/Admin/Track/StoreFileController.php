...
class StoreFileController extends Controller
{
    ...
    {
        $data = $request->validate(['file' => ['required'], 'local' => ['required']]);
        $file_name = $data['file']->getClientOriginalName();

        $path_file = Storage::putFileAs("public/excel", $data['file'], $file_name);

        File::create(['path' => $path_file, 'local' => $data['local']]);
